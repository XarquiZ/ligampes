...
export default function ChatPopup({
  ...
  const loadAvailableTeams = async () => {
    ...
    const otherTeamData = otherUserTeam?.teams;
    if (otherTeamData) {
      const team = Array.isArray(otherTeamData) ? otherTeamData[0] : otherTeamData;
      if (team) {
        teams.push({
          id: team.id,
          name: team.name,
  ...
  useEffect(() => {
    ...
    window.addEventListener('focusConversation', handleFocusConversation as unknown as EventListener);

    return () => {
      window.removeEventListener('focusConversation', handleFocusConversation as unknown as EventListener);
    };
  }, [conversations]);
