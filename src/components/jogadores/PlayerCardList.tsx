  ...
  return (
    ...
    >
      ...
      <div
        ...
        onClick={() => !isTransitioning && onToggle(player.id)}
      >
        {/* Foto do jogador */}
        <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-full overflow-hidden ring-3 lg:ring-4 ring-purple-500/50 flex-shrink-0">
          {player.photo_url ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-purple-700 to-pink-700 flex items-center justify-center">
              <span className="text-xl lg:text-2xl font-black text-white">{player.position}</span>
            </div>
          )}
        </div>

        {/* Informações principais */}
        <div className="flex-1 grid grid-cols-1 md:grid-cols-9 gap-3 lg:gap-4 text-xs lg:text-sm items-center">
          {/* Nome e Playstyle */}
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-2xl lg:text-3xl font-black text-yellow-400">
              {player.overall}
            </p>
          </div>
          
          {/* Botão de Comparação - NOVO */}
